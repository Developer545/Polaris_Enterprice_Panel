...
export default function LoginSteel({ companyId }: { companyId?: string }) {
  return (
    <LoginSplitBase companyId={companyId}>
      <style>{`
        @keyframes stripe-slide { from{background-position:0 0} to{background-position:56px 56px} }
      `}</style>
      <div style={{
        width: '100%', height: '100%',
        background: 'var(--lp-dk-bg,#090914)',
        backgroundImage: 'repeating-linear-gradient(45deg, var(--lp-a10,#7C3AED1A) 0, var(--lp-a10,#7C3AED1A) 1px, transparent 0, transparent 50%)',
        ...
        <div style={{ position: 'absolute', width: 320, height: 320, borderRadius: '50%', background: 'var(--lp-a20,#7C3AED33)', filter: 'blur(90px)', top: '-5%', right: '-5%', pointerEvents: 'none' }} />
